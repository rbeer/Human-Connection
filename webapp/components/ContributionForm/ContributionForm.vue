...
export default {
  ...
  data() {
    ...
    const { blurred: imageBlurred = false, aspectRatio: imageAspectRatio = null } = image || {}
  ...
  methods: {
    submit() {
      const { title, content, categoryIds, image } = this.formData
      if (image) {
        image.blurred = this.formData.imageBlurred
        if (this.imageUpload) {
